...
const Index: () => React$Node = (props) => {
  ...
  const setFilter = async () => {
    filterObj['PartTerminology'] = displayMake
    filterObj['Attributes'] = displayAttribute
  }
  ...
  const getData = (flag) => {
    ...
    SearchPart(tempObj, global).then(function (res) {
      ...
      let newDirectory = Object.values(res.data.get_data.reduce((acc, item) => {
        if (!acc[item.PartTerminology]) acc[item.PartTerminology] = {
          PartTerminology: item.PartTerminology,
          Position: item.Position,
          part: []
        };
        acc[item.PartTerminology].part.push(item);
        return acc;
      }, {}))
      if (flag == 'new') {
        setProducts(newDirectory);
      } else {
        setProducts([...products, ...newDirectory]);
      }
      setLoading(false);
    });
  };
        ...
        <Spinner status="primary" />
          ...
          <ScrollView
            ...
            {params.ref && refProducts.length > 0 ?
              <TouchableOpacity
                style={{ marginTop: hp(5), borderWidth: 0.5 }}>
                <View style={{ backgroundColor: 'rgb(23, 115, 51)', padding: wp(1) }}>
              ...
              : <></>}
            {
              ...
              products.map((HighPart) => (
                <TouchableOpacity
                  style={{ marginTop: hp(2), borderWidth: 0.5 }}>
                  <View style={styles.greenHeaderView}>
                    <Text style={styles.greenHeaderText}>
                      {HighPart.PartTerminology}
                    </Text>
                    <Text style={styles.greenHeaderText}>{HighPart.Position}</Text>
                  </View>

                  {HighPart && HighPart.part && HighPart.part.length && HighPart.part.map((part) => (
                    <TouchableOpacity
                      key={part._id}
                      onPress={() => {
                        goto(part._id);
                      ...
                      <View style={{ padding: wp(3) }}>
                        <View style={{ flexDirection: 'row' }}>
                          {part && part.images && part.images.length > 0 ? (
                            <Image
                              source={{
                          ...
                          <View style={{ left: wp(4), width: wp(52) }}>
                            <Text
